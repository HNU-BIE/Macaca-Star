import torch
import itertools
from utils.CycWaveM2D.utils.image_pool import ImagePool
from .base_model import BaseModel
from . import networks
from pytorch_msssim import ssim

from utils.CycWaveM2D.models.masked_l1_loss import masked_l1_loss
...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B','G_A_freq']
        ...
        if self.isTrain:  # define discriminators
            ...
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_B_freq = networks.define_WTD(opt.input_nc, opt.ndf, opt.init_type, opt.init_gain, self.gpu_ids)
        if self.isTrain:
        ...
        self.criterionSSIM = lambda pred, target: 1 - ssim(pred, target, data_range=1.0, size_average=True)
